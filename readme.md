...
It's built using Android Compose, ViewModel, Room, Pagination3, Navigation3, Room, Retrofit, Hilt, Coil, Firebase, Mockk, Turbine, Detekt and other modern libraries.

| Phone - Games | Phone - Game Details | Phone - Genres |
|---------|----------|----------|
...
2) Add RAWG api key to ```local.properties``` (located in the root of the project) in key named: ```RAWG_API_KEY``` like:

        gradle RAWG_API_KEY="key"

3) Make a new Firebase application and add property ```show_app_version_label``` in Firebase RemoteConfig (as you can see on the image: https://raw.githubusercontent.com/zsasko/rawg/main/docs/images/firebase_remote_config.png)
...
- App architecture is MVI where each viewmodel contains 'handleIntent' method which receives an intent and does some business logic.
    - StateFlow is used for notifying UI that data has been received.
    - Screens contain their state variables based on which appropriate layouts are displayed (error, success, loading).
- Selected genres are saved in local database.
    - When user clicks on genre, selected/deselected state is automatically saved in local Room db.
...
- Application layout is made for phones and tablets.
    ...
    - In tablet with lanscape mode, an game list and details of first (or last user-selected game) is displayed in list-details pane.
...
- For dependency injection a 'Hilt' library is used and for networking requests 'OkHttp' and 'Retrofit' are used
